...
// Process the complete form submission at once
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_notice'])) {
    try {
        // Begin transaction
        $conn->begin_transaction();
        
        // Sanitize and prepare all inputs
        $establishment = htmlspecialchars($_POST['establishment']);
        $owner_rep = htmlspecialchars($_POST['owner_representative'] ?? 'Not specified');
        $nature = ($_POST['nature_select'] === 'Others') 
            ? htmlspecialchars($_POST['nature_custom']) 
            : htmlspecialchars($_POST['nature_select']);
        $address = htmlspecialchars($_POST['address']);
        ...
        $fileContent .= "Notice Status: $notice_status\n";
        $fileContent .= "Remarks: $remarks\n";
        $fileContent .= "Issued By: $issuer_name\n";
        $fileContent .= "Position: $position\n";
        $fileContent .= "Witnessed By: $witnessed_by\n";
        ...
        $stmt = $conn->prepare("INSERT INTO establishments 
            (name, address, owner_representative, nature, products, violations, 
            notice_status, remarks, nov_files, issued_by, issued_datetime)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        // OPTIONAL: Insert notice status details if needed
        $stmt_status = $conn->prepare("INSERT INTO notice_stat 
            (id, status, issued_by, position, issued_datetime, witnessed_by, remarks)
            VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        // OPTIONAL: Process inventory products if available
        if (isset($_POST['products_inventory']) && is_array($_POST['products_inventory'])) {
            foreach ($_POST['products_inventory'] as $product) {
                $stmt_product = $conn->prepare("INSERT INTO inventory 
                    (id, product_name, sealed, withdrawn, description, price, pieces, 
                    dao_violation, other_violation, remarks)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
// Include header and output HTML
include '../templates/header.php';
?>
>>>>>>> Stashed changes
